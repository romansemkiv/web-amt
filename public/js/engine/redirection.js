// Intel AMT redirection transport (SOL / KVM / IDER) over the WebSocket relay.
var CreateAmtRedirect = function (module) {
    var obj = {};
    obj.m = module; // This is the inner module (Terminal, Desktop or IDER)
    module.parent = obj;
    ...
    obj.Start = function (host, port, user, pass, tls) {
        ...
        obj.connectstate = 0;
        obj.inDataCount = 0;
        obj.disconnectCode = null; // Set by Stop() or by the KVM module; read by the viewer UI.
        var url = window.location.protocol.replace('http', 'ws') + '//' + window.location.host + window.location.pathname.substring(0, window.location.pathname.lastIndexOf('/')) + '/relay?host=' + encodeURIComponent(host) + '&port=' + port + '&tls=' + tls;
    ...
    obj.xxOnSocketClosed = function () {
        if ((obj.inDataCount == 0) && (obj.tlsv1only == 0)) {
            // No data ever arrived — retry once forcing TLSv1 for old AMT firmware.
            obj.tlsv1only = 1;
            obj.socket = new WebSocket(window.location.protocol.replace('http', 'ws') + '//' + window.location.host + window.location.pathname.substring(0, window.location.pathname.lastIndexOf('/')) + '/relay?host=' + encodeURIComponent(obj.host) + '&port=' + obj.port + '&tls=' + obj.tls + '&tls1only=1');
            obj.socket.binaryType = 'arraybuffer';
            obj.socket.onopen = obj.xxOnSocketConnected;
            obj.socket.onmessage = obj.xxOnMessage;
            obj.socket.onclose = obj.xxOnSocketClosed;
    ...
    obj.Stop = function (x) {
        // Record why we stopped so the viewer can explain the failure. Codes set by the
        // KVM module (50000+) take precedence over the generic transport codes here.
        if ((x != null) && (obj.disconnectCode == null)) { obj.disconnectCode = x; }
        obj.xxStateChange(0);
        obj.connectstate = -1;
        obj.acc = null;
