var CreateAmtRemoteDesktop = function (divid, scrolldiv) {
        ...
        while ((obj.acc != null) && (obj.acc.byteLength > 0)) {
            ...
            if ((obj.state == 0) && (obj.acc.byteLength >= 12)) {
                ...
                */
                ...
                obj.fullRefreshPending = true; // WebAMT: first frame must be a full (non-incremental) update
                _SendRefresh();
                ...
                switch (obj.acc[0]) {
                    case 0: // FramebufferUpdate
                        ...
                        var _tileCount = accview.getUint16(2); // number of tiles in this frame
                        cmdsize = 4;
                        ...
                        if (_tileCount == 0) {
                            // WebAMT: an empty frame (static screen, e.g. BIOS/POST). The original
                            // code left state at 100 here and never asked again, wedging the loop.
                            // Stay in state 4 and poll for the next frame. After several consecutive
                            // empties, request one full (non-incremental) frame so a static screen
                            // stays painted — this never runs while the screen is actively updating,
                            // so it doesn't affect the frame rate on a live/OS desktop.
                            obj.emptyFrames = (obj.emptyFrames || 0) + 1;
                            if ((obj.emptyFrames % 8) === 0) { obj.fullRefreshPending = true; }
                            setTimeout(_SendRefresh, (obj.frameRateDelay > 0) ? obj.frameRateDelay : 200);
                        } else {
                            obj.emptyFrames = 0; // real tiles arrived — screen is active
                            obj.state = 100 + _tileCount; // add 100 and use that as our protocol state
                        }
    ...
    function _SendRefresh() {
        ...
        // WebAMT: the first request after connect (and any explicit "force redraw") must be
        // non-incremental so Intel AMT sends the full framebuffer. Static screens such as
        // BIOS/POST return nothing to an incremental request, which would leave the canvas blank.
        var incremental = (obj.fullRefreshPending === true) ? 0 : 1;
        obj.fullRefreshPending = false;

        // Request the entire screen
        obj.send(String.fromCharCode(3, incremental, 0, 0, 0, 0) + ShortToStr(obj.rwidth) + ShortToStr(obj.rheight)); // FramebufferUpdateRequest
    }
    ...
    */
    ...
    obj.mousemove = function (e, force) {
        ...
        // WebAMT: map the pointer to remote pixels using getBoundingClientRect + clientX/Y.
        // This is correct under CSS scaling, centering and container scrolling — unlike the
        // original offsetParent-chain + pageX math, which drifted once the canvas was scaled.
        e = e || window.event;
        var rect = Q(obj.canvasid).getBoundingClientRect();
        var ScaleFactorWidth = (obj.canvas.canvas.width / rect.width);
        var ScaleFactorHeight = (obj.canvas.canvas.height / rect.height);
        obj.mx = ((e.clientX - rect.left) * ScaleFactorWidth);
        obj.my = ((e.clientY - rect.top) * ScaleFactorHeight);
        if (e.addx) { obj.mx += e.addx; }
        if (e.addy) { obj.my += e.addy; }
