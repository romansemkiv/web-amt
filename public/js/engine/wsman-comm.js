...
var CreateWsmanComm = function (host, port, user, pass, tls) {
    ...
    obj.xxConnectHttpSocket = function () {
        ...
        obj.inDataCount = 0;
        obj.socketState = 1;
        obj.socket = new WebSocket(window.location.protocol.replace('http', 'ws') + '//' + window.location.host + window.location.pathname.substring(0, window.location.pathname.lastIndexOf('/')) + '/relay?host=' + encodeURIComponent(obj.host) + '&port=' + obj.port + '&tls=' + obj.tls + '&tls1only=' + obj.tlsv1only);
        ...
        while (true) {
            ...
            if (obj.socketParseState == 1) {
                ...
                if ((obj.socketXHeader['connection'] != undefined) && (obj.socketXHeader['connection'].toLowerCase() == 'close') && ((obj.socketXHeader['transfer-encoding'] == undefined) || (obj.socketXHeader['transfer-encoding'].toLowerCase() != 'chunked'))) {
                    ...
                    if (isNaN(csize)) { if (obj.socket) { obj.socket.close(); } return; } // Critical error, close the socket and exit.
    ...
    obj.CancelAllQueries = function (s) {
        ...
        if (obj.socket != null) { try { obj.socket.close(); } catch (e) { } obj.socket = null; obj.socketState = 0; }
    }
